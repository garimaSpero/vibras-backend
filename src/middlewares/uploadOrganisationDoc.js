...
const s3Storage = multerS3({
    ...
    key: (req, file, cb) => {
        ...
        const fname = nameSegments[nameSegments.length - 2];
        const ftype = nameSegments[nameSegments.length - 1];
        console.log('fname', fname);
        console.log('ftype', ftype);
        // const fileName = folderName + Date.now() + "_" + fieldName + "_" + originalname;
        const fileName = folderName + fname + "_" + Date.now() + "." + ftype;
        cb(null, fileName);
    }
});
...
const uploadOrganisationDoc = multer({
    storage: s3Storage,
...
module.exports = uploadOrganisationDoc;